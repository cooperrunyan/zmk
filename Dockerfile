...
RUN pip3 install --user -r /zmk/app/scripts/requirements.txt

COPY config /board/config

FROM builder as left
RUN west build -d /build -s /zmk/app -b "nice_nano_v2" -- -DZMK_CONFIG=/board/config -DSHIELD="corne_left"

FROM builder as right
RUN west build -d /build -s /zmk/app -b "nice_nano_v2" -- -DZMK_CONFIG=/board/config -DSHIELD="corne_right"
